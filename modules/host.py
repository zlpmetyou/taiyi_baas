...
class HostHandler(object):
    ...
    def delete(self, host):
        ...
        if host_type == WORKER_TYPE_DOCKER or host_type == WORKER_TYPE_SWARM:
            self.host_agents[host_type].delete(host.worker_api)

        elif host_type == WORKER_TYPE_VSPHERE:
            if host.status == "pending":
                return False
            vmuuid = host.vcparam[utils.VMUUID]
            vcip = host.vcparam[utils.VCIP]
            vcusername = host.vcparam[utils.VCUSERNAME]
            vcpwd = host.vcparam[utils.VCPWD]
            vcport = host.vcparam[utils.VCPORT]
    ...
    def refresh_status(self, id):
        ...
        if not host:
            logger.warning("No host found with id=" + id)
            return False
        if not self.host_agents[host.host_type]\
    ...
    def _serialize(self, doc, keys=['id', 'name', 'worker_api', 'capacity',
                                    'host_type', 'create_ts', 'status', 'autofill',
