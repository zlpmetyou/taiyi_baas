...
setOrdererGlobals() {
    ...
    ORDERER=$3
        ...
        CORE_PEER_TLS_ROOTCERT_FILE=/etc/hyperledger/fabric/crypto-config/ordererOrganizations/${DOMAIN}/orderers/${ORDERER}.${DOMAIN}/msp/tlscacerts/tlsca.${DOMAIN}-cert.pem
        CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/fabric/crypto-config/ordererOrganizations/${DOMAIN}/users/Admin@${DOMAIN}/msp
}
...
fetchChannelConfig() {
  ...
  setOrdererGlobals orderer $DOMAIN $ORDERER

  echo "Fetching the most recent configuration block for the channel"
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config ${CHANNEL}.pb -o orderer.example.com:7050 -c ${CHANNEL} --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config ${CHANNEL}.pb -o orderer.example.com:7050 -c ${CHANNEL} --tls --cafile $ORDERER_CA
    set +x
  fi
