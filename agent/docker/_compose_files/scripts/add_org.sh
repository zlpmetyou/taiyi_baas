...
#	${configtxgen} -printOrg ${ORGMSP} -profile ./configtx.yaml > ./${ORGMSP}.json
    ...
    CHANNEL=$2
    set -x
    jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"'${ORG^}'MSP":.[1]}}}}}' ${CHANNEL}.json /tmp/channel-artifacts/${ORG}.json > ${CHANNEL}_config.json
...
#jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org2MSP":.[1]}}}}}' channel1.json /tmp/channel-artifacts/org2.json
