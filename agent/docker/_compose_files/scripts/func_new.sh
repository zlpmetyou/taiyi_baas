...
setEnvs () {
	...
	local peer=$2  # peer_name
	[ -z $org ] && [ -z $peer ] && echo_r "input param invalid" && exit -1

	local t=""
	export CORE_PEER_LOCALMSPID="${org^}MSP"    #org23be49e644f347efa2b047429df69b7b
	#CORE_PEER_MSPCONFIGPATH=\$${ORG${org}_ADMIN_MSP}
	#org23be49e644f347efa2b047429df69b7b_peer143fdad9a18e4f1a917ea9f903962080_url
	t="\${${org^^}_${peer^^}_URL}" && export CORE_PEER_ADDRESS=`eval echo $t`
	t="\${${org^^}_ADMIN_MSP}" && export CORE_PEER_MSPCONFIGPATH=`eval echo $t`
	t="\${${org^^}_${peer^^}_TLS_ROOTCERT}" && export CORE_PEER_TLS_ROOTCERT_FILE=`eval echo $t`
...
channelJoinWithRetry () {
	...
	local rc=$?
	while [ ${counter} -lt ${MAX_RETRY} -a ${rc} -ne 0 ]; do
		echo "${peer} failed to join channel ${channel}, retry after 2s"
		sleep 2
		channelJoinAction ${channel}
		rc=$?
		let counter=${counter}+1
