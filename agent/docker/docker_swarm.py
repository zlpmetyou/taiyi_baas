...
from common import log_handler, LOG_LEVEL, MAP_PATH
from common import \
    WORKER_TYPES, BLOCKCHAIN_CONFIG_FILES_PATH, \
...
# COMPOSE_FILE_PATH = os.getenv("COMPOSE_FILE_PAT",
#                               "." + os.sep + "agent" + os.sep + "docker" +
#                               os.sep + "_compose_files")

COMPOSE_FILE_PATH = BLOCKCHAIN_CONFIG_FILES_PATH
...
def check_worker_api(worker_api, timeout=2):
    if not worker_api:
        return None
    url = urlparse(worker_api)
    if not url.scheme:
    ...
    if len(segs) != 3:
        ...
        return None
    try:
        Client(base_url=worker_api, version="auto", timeout=timeout)
    except Exception as e:
        logger.error(e)
        return False
    else:
...
# /home/zlp/PycharmProjects/baas/src/agent/docker/_compose_files/fabric-1.0
...
def generate_config_file(path, name, data):
    file_path = path + os.sep + name
...
def _compose_set_env(name, worker_api, mapped_ports=SERVICE_PORTS,
    ...
    envs = {
        ...
        'CLUSTER_NETWORK': "{}_default".format(name),
        'DOCKER_HOST': worker_api,
        'PEER_NETWORKID': name,
        'NETWORK_TYPES': config['network_type'],
        'VM_ENDPOINT': worker_api,
        'VM_DOCKER_HOSTCONFIG_NETWORKMODE': "{}_default".format(name),
        'HLF_VERSION': HLF_VERSION,
    }
    if config['network_type'] in NETWORK_TYPES:
        envs.update({
            'COMPOSE_FILE': "{}.yaml".format(name),
            'COMPOSE_PROJECT_PATH': MAP_PATH,
        })
    elif config['network_type'] == NETWORK_TYPE_FABRIC_PRE_V1:
        envs.update({
            'COMPOSE_FILE': "fabric-{}.yml".format(config['peer']),
            'PEER_VALIDATOR_CONSENSUS_PLUGIN': config['consensus_plugin'],
            'PBFT_GENERAL_MODE': config['consensus_mode'],
            'PBFT_GENERAL_N': str(config['peer']),
        })
...
def compose_up(name, host, mapped_ports, config=None, timeout=5):
    ...
    try:
        template_path = BLOCKCHAIN_CONFIG_FILES_PATH + os.sep + name
...
def compose_down(name, worker_api, mapped_ports=SERVICE_PORTS,
    ...
    project = get_project(COMPOSE_FILE_PATH + os.sep + name)
