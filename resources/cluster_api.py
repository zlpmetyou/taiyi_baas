...
from .clusters import ClusterView
# from .organization import PeerOrgView
...
bp_cluster_api = Blueprint('bp_cluster_api', __name__,
                           url_prefix='/{}'.format("v0"))

front_rest_v2 = Blueprint('front_rest_v2', __name__,
                          url_prefix='/{}'.format("v2"))

cluster_api = Api(bp_cluster_api)
cluster_api.add_resource(ClusterView, '/clusters')
# cluster_api.add_resource(PeerOrgView, '/org')
...
def cluster_actions():
    ...
    if action == "apply":
        ...
        cluster_release(r)
        return jsonify(stat=200, errmsg='成功')
    elif action == "start":
